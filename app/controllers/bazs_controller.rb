class BazsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @baz.save
        ...
        format.json { render :show, status: :created, location: @baz }
      else
  ...
  def update
    respond_to do |format|
      if @baz.update(baz_params)
        ...
        format.json { render :show, status: :ok, location: @baz }
      else
  ...
  def destroy
    @baz.destroy
    respond_to do |format|
      format.html { redirect_to bazs_url, notice: 'Baz was successfully destroyed.' }
  ...
  private
    ...
    def baz_params
      params.require(:baz).permit(:name)
    end
end
